...
FReply FFindInFlowResult::OnDoubleClick() const
{
	if (bIsSubGraphNode && Parent.IsValid())
	{
		...
		{
			if (UFlowNode_AbstractSubGraph* SubGraph = Cast<UFlowNode_AbstractSubGraph>(ParentNode->GetFlowNodeBase()))
			{
...
bool SFindInFlow::RecurseIntoSubgraphsIfEnabled(UEdGraphNode* EdNode, FSearchResult ParentResult, const TArray<FString>& Tokens, int32 Depth)
{
	...
	UFlowNode_AbstractSubGraph* SubGraph = Cast<UFlowNode_AbstractSubGraph>(FlowGraphNode->GetFlowNodeBase());
	if (!SubGraph)
	{
		return false;
	}
